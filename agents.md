...
- **Растяжение при драге — НЕ через поворот.** Версия с `rotate(angle) → scaleX/scaleY →
  rotate(-angle)` (выравнивание по вектору скорости) заменена целиком после реального использования
  (см. `roadmap.md` Sprint 12): `atan2(vy,vx)` шумит на низкой скорости (пиксельное квантование даёт
  никакое соотношение сигнал/шум), поворот на шумный угол выглядел дёргано, а комбинация трёх
  поворотов/скейлов на элементах, которыми управляет framer-motion, в части случаев оставляла ноду
  залипшей растянутой. Текущая схема (`StepNode.tsx`) — без поворотов: скорость сглаживается EMA
  (`VELOCITY_SMOOTHING`), `bias=(|vx|-|vy|)/(|vx|+|vy|)` определяет, какая ось растягивается, а
  какая сжимается **навстречу** (`scaleX=1+stretchMag*bias`, `scaleY=1-stretchMag*bias`) — это и
  гасит шум на низкой скорости (умножается на малый `stretchMag`), и исключает разнос осей
  арифметически. Не возвращать поворотный вариант без крайней необходимости.
- **Clamp растяжения — внутри `useTransform`, не только при `.set()`.** Отображаемые `scaleX/scaleY`
  — производные через `useTransform(raw, v => clamp(v, MIN, MAX))`, а не сами `raw`-motion-values,
  которые двигает `animate()`/эффект. Это гарантия, что даже перелёт пружины осседания за границы не
  даст видимую инверсию/"залипание" ноды. Просто ограничивать значение при записи мотион-вэлью
  недостаточно (сам `animate()` не знает о ваших границах и может временно увести значение за них
  при низком damping) — граница обязана быть в производном (рендерящемся) значении.
- **`whileHover`/`whileTap` должны быть отключены (`dragging ? undefined : {...}`), пока xyflow
  ...
  перетаскивания и накладывают свои transform-значения поверх любой кастомной драг-анимации.
  Держите это правило в голове при добавлении новых hover/tap-эффектов на ноду.
