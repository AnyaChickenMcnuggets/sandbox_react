...
├── graph/                     # DAG-движок: ноды/рёбра/маппинг/layout — общий для editor и monitor
...
- **`localStorage`** — только layout нод (`layoutStorage.ts`). Не использовать для чего-либо ещё
  без явной причины — это осознанно единственное исключение из "источник истины — сервер".
- **Модульный стор (`toastStore.ts`)** — для состояния, которое нужно мутировать из кода вне React
  дерева (`queryClient.ts` создаётся до рендера). Не заводить второй такой стор без необходимости —
  если состояние доступно из компонента, обычный `useState`/React Query предпочтительнее.
