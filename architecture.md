...
**Версия с поворотом (rotate + scaleX/scaleY в трёх вложенных слоях, align вдоль вектора скорости)
не прижилась** — после реального использования оказалось, что `atan2(vy,vx)` на низкой скорости
шумит на уровне пиксельного квантования (единичные `dx`/`dy` — соотношение сигнал/шум никакое), и
поворот на этот шумный угол выглядел "дёрганым" даже при плавном медленном драге; отдельно,
комбинация `rotate`+`scaleX/scaleY`+`rotate(-angle)` на элементах, которыми управляет framer-motion,
в части случаев оставляла ноду растянутой навсегда после отпускания. Подход заменён целиком (не
дочинен) — см. `roadmap.md`, Sprint 12.

**Текущее решение — без единого поворота.** Скорость (`vx`, `vy` — `Δposition/Δt` между соседними
вызовами `useEffect` на `positionAbsoluteX/Y`, поля `NodeProps` из `@xyflow/react`, обновляются на
каждый тик реального драга xyflow) сглаживается экспоненциальным средним
(`VELOCITY_SMOOTHING = 0.75`) — гасит покадровый пиксельный шум на медленном движении, не убивая
отклик на резких рывках. `bias = (|vx|-|vy|) / (|vx|+|vy|)` (∈[-1..1]) показывает, какая ось
доминирует; `stretchMag` — общая величина растяжения от модуля скорости. Обе оси двигаются
**навстречу друг другу** от одного и того же `bias`:
...
Это математически исключает одновременный разнос обеих осей в одну сторону (в отличие от старой
формулы с независимыми `stretchX`/`stretchY` и перекрёстными членами). Шум в `bias` на низкой
скорости умножается на маленький `stretchMag` (тоже малый при низкой скорости) — визуальный эффект
шума гасится сам, без явного порога/dead zone.

**Clamp — внутри `useTransform`, отдельно от записи motion value.** `scaleX/scaleY`, которые реально
идут в `style`, — это `useTransform(rawScaleX, v => clamp(v, 0.6, 1.5))` (и аналогично для Y): даже
если пружина осседания (`animate(rawScaleX, 1, {stiffness:260, damping:16})`) на отпускании
перелетит через целевое значение, отображаемый scale не выйдет за безопасный диапазон — "залипание"
в растянутом виде математически невозможно, а не просто маловероятно.
...
scale/scaleX/scaleY поверх динамического сквоша от скорости. Проверка `dragging ? undefined :
{...}` на `whileHover`/`whileTap` полностью убирает конфликт.
...
ноду) соседняя нода получает `collisionBus.bump(neighborId)` — короткий импульс на тех же
`rawScaleX/rawScaleY` (без направления/угла — упрощено вместе с отказом от поворотов).
